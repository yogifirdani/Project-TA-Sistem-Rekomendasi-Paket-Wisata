...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white/90">Tambah Artikel Baru</h1>
            <p class="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Tulis artikel menarik untuk pengunjung Kutamasya</p>
        </div>
    </div>
    ...
    <form action="{{ route('admin.kelola-artikel.store') }}" method="POST" enctype="multipart/form-data" x-data="{ activeTab: 'id' }">
        @csrf
        ...
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="text-xs text-gray-500 dark:text-gray-400 italic">
                <span x-show="activeTab === 'id'">Menulis dalam Bahasa Indonesia</span>
                <span x-show="activeTab === 'en'">Writing in English</span>
            </div>
        </div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div x-show="activeTab === 'id'">
                            ...
                            <input type="text" name="title" value="{{ old('title') }}"
                                placeholder="Contoh: 5 Tempat Wisata Hits di Banyuwangi"
                        ...
                        <div x-show="activeTab === 'en'" x-cloak>
                            ...
                            <input type="text" name="title_en" value="{{ old('title_en') }}"
                                placeholder="Example: 5 Trending Tourist Spots in Banyuwangi"
                                class="shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                        </div>
                        ...
                        <div x-show="activeTab === 'id'">
                            ...
                            <textarea id="editor-content" name="content">{{ old('content') }}</textarea>
                        </div>
                        <div x-show="activeTab === 'en'" x-cloak>
                            <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                Content (English)
                            </label>
                            <textarea id="editor-content-en" name="content_en">{{ old('content_en') }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-1 space-y-6">
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-white/[0.03]">
                    ...
                    <select name="status" class="shadow-theme-xs h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90">
                        <option value="active" {{ old('status') == 'active' ? 'selected' : '' }}>Aktif (Tampil)</option>
                        <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }}>Draft (Sembunyi)</option>
                    </select>
                </div>
