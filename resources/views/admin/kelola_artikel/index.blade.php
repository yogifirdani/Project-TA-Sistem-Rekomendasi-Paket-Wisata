...
<div class="space-y-6" x-data="{ deleteModalOpen: false, deleteActionUrl: '', deleteItemName: '', isDeleting: false }" x-init="$watch('deleteModalOpen', value => { if (!value) isDeleting = false })">
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800 dark:text-white/90">Kelola Artikel</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Daftar semua artikel yang akan tampil di halaman depan</p>
        </div>
        <a href="{{ route('admin.kelola-artikel.create') }}"
    ...
    <div id="table-container" class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        @include('admin.kelola_artikel._table')
    </div>
